 ...
 */
class rwu_formatinvoicepdf_oxorder extends rwu_formatinvoicepdf_oxorder_parent
{
    ...
    protected function _setBillingAddressToPdf($oPdf)
    {
		...
		*/
		$oPdf->text(15, 60, $this->oxorder__oxbillcompany->getRawValue());
		$oPdf->text(15, 64, $this->oxorder__oxbillfname->getRawValue() . ' ' . $this->oxorder__oxbilllname->getRawValue());
		$oPdf->text(15, 68, $this->oxorder__oxbillstreet->getRawValue() . ' ' . $this->oxorder__oxbillstreetnr->value);
		$oPdf->text(15, 72, $this->oxorder__oxbillzip->value . ' ' . $this->oxorder__oxbillcity->getRawValue());
    }
    ...
    protected function _setDeliveryAddressToPdf($oPdf)
    {
		...
		*/
		$oPdf->text(15, 91, $this->oxorder__oxdelcompany->getRawValue());
		$oPdf->text(15, 95, $this->oxorder__oxdelfname->getRawValue() . ' ' . $this->oxorder__oxdellname->getRawValue());
		$oPdf->text(15, 99, $this->oxorder__oxdelstreet->getRawValue() . ' ' . $this->oxorder__oxdelstreetnr->value);
		$oPdf->text(15, 103, $this->oxorder__oxdelzip->value . ' ' . $this->oxorder__oxdelcity->getRawValue());
    }
    ...
    protected function _setOrderArticlesToPdf($oPdf, &$iStartPos, $blShowPrice = true)
    {
        ...
        foreach ($this->_oArticles as $key => $oOrderArt) {
			...
			$productTitle = ($oOrderArt->oxorderarticles__oxselvariant->value)? 
					substr(strip_tags($this->_replaceExtendedChars($oOrderArt->oxorderarticles__oxtitle->getRawValue(), true)), 0, 58) . ' ' . substr($oOrderArt->oxorderarticles__oxselvariant->value, 0, 58) 
					: 
						substr(strip_tags($this->_replaceExtendedChars($oOrderArt->oxorderarticles__oxtitle->getRawValue(), true)) ,0, 58);
			$oPdf->text(45, $iStartPos, $productTitle);
    ...
    public function exportStandart($oPdf)
    {
        ...
        // shop information
        $oPdf->setFont($oPdfBlock->getFont(), '', 6);
		$oPdf->text(15, 55, strip_tags($oShop->oxshops__oxcompany->getRawValue()) . ' ' . strip_tags($oShop->oxshops__oxfname->getRawValue()) . ' ' . strip_tags($oShop->oxshops__oxlname->getRawValue()) . ' ' . strip_tags($oShop->oxshops__oxstreet->getRawValue()) . ' ' . strip_tags($oShop->oxshops__oxzip->value) . ' ' . strip_tags($oShop->oxshops__oxcity->getRawValue()));
        ...
        // order number
        //$sText = $this->translate( 'ORDER_OVERVIEW_PDF_PURCHASENR' ).' '.$this->oxorder__oxordernr->value;
        //$oPdf->text( 195 - $oPdf->getStringWidth( $sText ), 63, $sText);
        // bill number
        //$sText = $this->translate( 'ORDER_OVERVIEW_PDF_COUNTNR' ).' '.$this->oxorder__oxbillnr->value;
        //$oPdf->text( 195 - $oPdf->getStringWidth( $sText ), 67, $sText );
        ...
        // generating pdf file
        if (class_exists('rwu_formatinvoicepdf_invoicepdfarticlesummary')) {
            $oArtSumm = new rwu_formatinvoicepdf_invoicepdfarticlesummary($this, $oPdf);
        } else {
            $oArtSumm = new InvoicepdfArticleSummary($this, $oPdf);
        }

        $iHeight = $oArtSumm->generate($siteH);
		//$oPdf->text(100, $siteH, $iHeight. ' / '. $siteH);
        if ($siteH + $iHeight > 270) {
            $this->pdfFooter($oPdf);
        ...
        // bank account data for payadvance
        if (($this->oxorder__oxpaymenttype->value == 'oxidpayadvance') && ($this->oxorder__oxpaid->value == "0000-00-00 00:00:00")) {
            $oPdf->setFont($oPdfBlock->getFont(), 'B', 8);
            $oPdf->text(15, $siteH, $this->translate( 'ORDER_OVERVIEW_PDF_ADD_PAYINFO' ));
            $oPdf->setFont($oPdfBlock->getFont(), '', 8);
            $oPdf->text(15, $siteH += 6, $this->translate( 'ORDER_OVERVIEW_PDF_ADD_OWNER' ));
            $oPdf->text(45, $siteH, strip_tags( $oShop->oxshops__oxlname->getRawValue()));
            $oPdf->text(15, $siteH += 4, $this->translate( 'ORDER_OVERVIEW_PDF_ADD_BANKNAME' ));
            $oPdf->text(45, $siteH, strip_tags( $oShop->oxshops__oxbankname->getRawValue() ));
            $oPdf->text(15, $siteH += 4, $this->translate( 'ORDER_OVERVIEW_PDF_ACCOUNTNR' ));
            $oPdf->text(45, $siteH, strip_tags( $oShop->oxshops__oxibannumber->value ));
            $oPdf->text(15, $siteH += 4, $this->translate( 'ORDER_OVERVIEW_PDF_BANKCODE' ));
            $oPdf->text(45, $siteH, strip_tags( $oShop->oxshops__oxbiccode->value ));
            $oPdf->text(15, $siteH += 4, $this->translate( 'ORDER_OVERVIEW_PDF_ADD_PAYTEXT' ));
            $oPdf->text(45, $siteH, $this->translate( 'ORDER_OVERVIEW_PDF_ADD_PAYTEXT_COUNTNR' ).$this->oxorder__oxordernr->value.' / '.$this->translate( 'ORDER_OVERVIEW_PDF_ADD_PAYTEXT_CUSTNR' ).$oUser->oxuser__oxcustnr->value);

            //$oPdf->text(15, $siteH += 6, $this->translate( 'ORDER_OVERVIEW_PDF_ADD_PAYTARGET' ));
            //$oPdf->text(15 + $oPdf->getStringWidth($this->translate( 'ORDER_OVERVIEW_PDF_ADD_PAYTARGET' )), $siteH, date( 'd.m.Y', strtotime( '+' . $this->getPaymentTerm() . ' day', strtotime( $this->oxorder__oxbilldate->value ) ) ) );
            $siteH += 8;
        }
		$oPdf->setFont($oPdfBlock->getFont(), '', 8);
